...
// Copyright 2023 by Uniontech Software Technology Co., Ltd. Port html to pure javascript language.
...
#define SCLEX_JS (SCLEX_JAVANOCASE + 1)
...
const char * const jsWordListDesc[] = {
    "",
	"JavaScript keywords",
    "",
    "",
...
struct OptionSetHTML : public OptionSet<OptionsHTML> {
	OptionSetHTML(bool isPHPScript_) {
        ...
        DefineWordListSets(isPHPScript_ ? phpscriptWordListDesc : jsWordListDesc);
	}
};
...
class LexerJS : public DefaultLexer {
	bool isXml;
	bool isPHPScript;
	WordList keywords;
	WordList keywords2;
...
public:
	...
	static ILexer5 *LexerFactoryHTML() {
        return new LexerJS(false, false);
	}
    static ILexer5 *LexerFactoryJS() {
        return new LexerJS(false, false);
    }
	static ILexer5 *LexerFactoryXML() {
        return new LexerJS(true, false);
	}
	static ILexer5 *LexerFactoryPHPScript() {
        return new LexerJS(false, true);
	}
};

Sci_Position SCI_METHOD LexerJS::PropertySet(const char *key, const char *val) {
...
Sci_Position SCI_METHOD LexerJS::WordListSet(int n, const char *wl) {
...
void SCI_METHOD LexerJS::Lex(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument *pAccess) {
	...
	for (Sci_Position i = startPos; i < lengthDoc; i++) {
        ...
        chNext2 = static_cast<unsigned char>(styler.SafeGetCharAt(i + 2));
        printf("---%d:%c %c %c", static_cast<int>(i), ch, chNext, chNext2);
...
LexerModule lmJS(SCLEX_JS, LexerJS::LexerFactoryJS, "js", jsWordListDesc);
