...
constexpr bool IsSpaceEquiv(int state) noexcept {
    return (state <= SCE_JAVA_COMMENTDOC) ||
        // including SCE_JAVA_DEFAULT, SCE_JAVA_COMMENT, SCE_JAVA_COMMENTLINE
        (state == SCE_JAVA_COMMENTLINEDOC) || (state == SCE_JAVA_COMMENTDOCKEYWORD) ||
        (state == SCE_JAVA_COMMENTDOCKEYWORDERROR);
}
...
void highlightTaskMarker(StyleContext &sc, LexAccessor &styler,
	...
	if ((isoperator(sc.chPrev) || IsASpace(sc.chPrev)) && markerList.Length()) {
		...
		if (markerList.InList(marker)) {
            sc.SetState(SCE_JAVA_TASKMARKER|activity);
		}
	}
}
...
constexpr bool IsStreamCommentStyle(int style) noexcept {
    return style == SCE_JAVA_COMMENT ||
        style == SCE_JAVA_COMMENTDOC ||
        style == SCE_JAVA_COMMENTDOCKEYWORD ||
        style == SCE_JAVA_COMMENTDOCKEYWORDERROR;
}
...
const char *const javaWordLists[] = {
...
struct OptionSetJAVA : public OptionSet<OptionsJAVA> {
    OptionSetJAVA() {
        DefineProperty("styling.within.preprocessor", &OptionsJAVA::stylingWithinPreprocessor,
			"For C++ code, determines whether all preprocessor code is styled in the "
			"preprocessor style (0, the default) or only from the initial # to the end "
			"of the command word(1).");

        DefineProperty("lexer.java.allow.dollars", &OptionsJAVA::identifiersAllowDollars,
            "Set to 0 to disallow the '$' character in identifiers with the java lexer.");
        ...
        DefineProperty("lexer.java.update.preprocessor", &OptionsJAVA::updatePreprocessor,
			"Set to 1 to update preprocessor definitions when #define found.");

        DefineProperty("lexer.java.verbatim.strings.allow.escapes", &OptionsJAVA::verbatimStringsAllowEscapes,
			"Set to 1 to allow verbatim strings to contain escape sequences.");

        DefineProperty("lexer.java.triplequoted.strings", &OptionsJAVA::triplequotedStrings,
			"Set to 1 to enable highlighting of triple-quoted strings.");

        DefineProperty("lexer.java.hashquoted.strings", &OptionsJAVA::hashquotedStrings,
			"Set to 1 to enable highlighting of hash-quoted strings.");

        DefineProperty("lexer.java.backquoted.strings", &OptionsJAVA::backQuotedStrings,
			"Set to 1 to enable highlighting of back-quoted raw strings .");

        DefineProperty("lexer.java.escape.sequence", &OptionsJAVA::escapeSequence,
			"Set to 1 to enable highlighting of escape sequences in strings");

        DefineProperty("fold", &OptionsJAVA::fold);

        DefineProperty("fold.java.syntax.based", &OptionsJAVA::foldSyntaxBased,
			"Set this property to 0 to disable syntax based folding.");

        DefineProperty("fold.comment", &OptionsJAVA::foldComment,
            "This option enables folding multi-line comments and explicit fold points when using the Java lexer. "
			"Explicit fold points allows adding extra folding by placing a //{ comment at the start and a //} "
			"at the end of a section that should fold.");

        DefineProperty("fold.java.comment.multiline", &OptionsJAVA::foldCommentMultiline,
			"Set this property to 0 to disable folding multi-line comments when fold.comment=1.");

        DefineProperty("fold.java.comment.explicit", &OptionsJAVA::foldCommentExplicit,
			"Set this property to 0 to disable folding explicit fold points when fold.comment=1.");

        DefineProperty("fold.java.explicit.start", &OptionsJAVA::foldExplicitStart,
			"The string to use for explicit fold start points, replacing the standard //{.");

        DefineProperty("fold.java.explicit.end", &OptionsJAVA::foldExplicitEnd,
			"The string to use for explicit fold end points, replacing the standard //}.");

        DefineProperty("fold.java.explicit.anywhere", &OptionsJAVA::foldExplicitAnywhere,
			"Set this property to 1 to enable explicit fold points anywhere, not just in line comments.");

        DefineProperty("fold.java.preprocessor.at.else", &OptionsJAVA::foldPreprocessorAtElse,
			"This option enables folding on a preprocessor #else or #endif line of an #if statement.");

        DefineProperty("fold.preprocessor", &OptionsJAVA::foldPreprocessor,
            "This option enables folding preprocessor directives when using the Java lexer. "
			"Includes C#'s explicit #region and #endregion folding directives.");

        DefineProperty("fold.compact", &OptionsJAVA::foldCompact);

        DefineProperty("fold.at.else", &OptionsJAVA::foldAtElse,
            "This option enables Java folding on a \"} else {\" line of an if statement.");

        DefineWordListSets(javaWordLists);
	}
};

const char styleSubable[] = {SCE_JAVA_IDENTIFIER, SCE_JAVA_COMMENTDOCKEYWORD, 0};

LexicalClass lexicalClasses[] = {
    // Lexer Java SCLEX_JAVA SCE_JAVA_:
    0, "SCE_JAVA_DEFAULT", "default", "White space",
    1, "SCE_JAVA_COMMENT", "comment", "Comment: /* */.",
    2, "SCE_JAVA_COMMENTLINE", "comment line", "Line Comment: //.",
    3, "SCE_JAVA_COMMENTDOC", "comment documentation", "Doc comment: block comments beginning with /** or /*!",
    4, "SCE_JAVA_NUMBER", "literal numeric", "Number",
    5, "SCE_JAVA_WORD", "keyword", "Keyword",
    6, "SCE_JAVA_STRING", "literal string", "Double quoted string",
    7, "SCE_JAVA_CHARACTER", "literal string character", "Single quoted string",
    8, "SCE_JAVA_UUID", "literal uuid", "UUIDs (only in IDL)",
    9, "SCE_JAVA_PREPROCESSOR", "preprocessor", "Preprocessor",
    10, "SCE_JAVA_OPERATOR", "operator", "Operators",
    11, "SCE_JAVA_IDENTIFIER", "identifier", "Identifiers",
    12, "SCE_JAVA_STRINGEOL", "error literal string", "End of line where string is not closed",
    13, "SCE_JAVA_VERBATIM", "literal string multiline raw", "Verbatim strings for C#",
    14, "SCE_JAVA_REGEX", "literal regex", "Regular expressions for JavaScript",
    15, "SCE_JAVA_COMMENTLINEDOC", "comment documentation line", "Doc Comment Line: line comments beginning with /// or //!.",
    16, "SCE_JAVA_WORD2", "identifier", "Keywords2",
    17, "SCE_JAVA_COMMENTDOCKEYWORD", "comment documentation keyword", "Comment keyword",
    18, "SCE_JAVA_COMMENTDOCKEYWORDERROR", "error comment documentation keyword", "Comment keyword error",
    19, "SCE_JAVA_GLOBALCLASS", "identifier", "Global class",
    20, "SCE_JAVA_STRINGRAW", "literal string multiline raw", "Raw strings for C++0x",
    21, "SCE_JAVA_TRIPLEVERBATIM", "literal string multiline raw", "Triple-quoted strings for Vala",
    22, "SCE_JAVA_HASHQUOTEDSTRING", "literal string", "Hash-quoted strings for Pike",
    23, "SCE_JAVA_PREPROCESSORCOMMENT", "comment preprocessor", "Preprocessor stream comment",
    24, "SCE_JAVA_PREPROCESSORCOMMENTDOC", "comment preprocessor documentation", "Preprocessor stream doc comment",
    25, "SCE_JAVA_USERLITERAL", "literal", "User defined literals",
    26, "SCE_JAVA_TASKMARKER", "comment taskmarker", "Task Marker",
    27, "SCE_JAVA_ESCAPESEQUENCE", "literal string escapesequence", "Escape sequence",
};
...
class LexerJAVA : public ILexer5 {
	bool caseSensitive;
...
public:
    explicit LexerJAVA(bool caseSensitive_) :
    ...
    // Deleted so LexerJAVA objects can not be copied.
    LexerJAVA(const LexerJAVA &) = delete;
    LexerJAVA(LexerJAVA &&) = delete;
    void operator=(const LexerJAVA &) = delete;
	...
	const char * SCI_METHOD PropertyNames() override {
        return osJava.PropertyNames();
	}
	int SCI_METHOD PropertyType(const char *name) override {
        return osJava.PropertyType(name);
	}
	const char * SCI_METHOD DescribeProperty(const char *name) override {
        return osJava.DescribeProperty(name);
	}
	Sci_Position SCI_METHOD PropertySet(const char *key, const char *val) override;
	const char * SCI_METHOD DescribeWordListSets() override {
        return osJava.DescribeWordListSets();
	}
    ...
    const char * SCI_METHOD GetName() override {
        return caseSensitive ? "java" : "javanocase";
    }
    int SCI_METHOD  GetIdentifier() override {
        return caseSensitive ? SCLEX_JAVA : SCLEX_JAVANOCASE;
    }
	const char * SCI_METHOD PropertyGet(const char *key) override;

    static ILexer5 *LexerFactoryJAVA() {
        return new LexerJAVA(true);
	}
    static ILexer5 *LexerFactoryJAVAInsensitive() {
        return new LexerJAVA(false);
	}
...
Sci_Position SCI_METHOD LexerJAVA::PropertySet(const char *key, const char *val) {
    if (osJava.PropertySet(&options, key, val)) {
        if (strcmp(key, "lexer.java.allow.dollars") == 0) {
...
const char * SCI_METHOD LexerJAVA::PropertyGet(const char *key) {
    return osJava.PropertyGet(key);
}

Sci_Position SCI_METHOD LexerJAVA::WordListSet(int n, const char *wl) {
...
void SCI_METHOD LexerJAVA::Lex(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument *pAccess) {
    ...
    int styleBeforeDCKeyword = SCE_JAVA_DEFAULT;
    int styleBeforeTaskMarker = SCE_JAVA_DEFAULT;
    ...
    if ((MaskActive(initStyle) == SCE_JAVA_PREPROCESSOR) ||
      (MaskActive(initStyle) == SCE_JAVA_COMMENTLINE) ||
      (MaskActive(initStyle) == SCE_JAVA_COMMENTLINEDOC)) {
	...
	if (startPos > 0) {
		...
		while (--back && IsSpaceEquiv(MaskActive(styler.StyleAt(back))))
			;
        if (MaskActive(styler.StyleAt(back)) == SCE_JAVA_OPERATOR) {
    ...
    const WordClassifier &classifierIdentifiers = subStyles.Classifier(SCE_JAVA_IDENTIFIER);
    const WordClassifier &classifierDocKeyWords = subStyles.Classifier(SCE_JAVA_COMMENTDOCKEYWORD);
	...
	for (; sc.More();) {
		...
		if (sc.atLineStart) {
            ...
            if ((sc.state == SCE_JAVA_STRING) || (sc.state == SCE_JAVA_CHARACTER)) {
                // Prevent SCE_JAVA_STRINGEOL from leaking back to previous line which
				// ends with a line continuation by locking in the state up to this position.
				sc.SetState(sc.state);
			}
            if ((MaskActive(sc.state) == SCE_JAVA_PREPROCESSOR) && (!continuationLine)) {
                sc.SetState(SCE_JAVA_DEFAULT|activitySet);
			}
		...
		switch (MaskActive(sc.state)) {
            ...
            case SCE_JAVA_NUMBER:
				...
				if (sc.ch == '_') {
                    sc.ChangeState(SCE_JAVA_USERLITERAL|activitySet);
            ...
            case SCE_JAVA_IDENTIFIER:
				if (sc.atLineStart || sc.atLineEnd || !setWord.Contains(sc.ch) || (sc.ch == '.')) {
					...
					if (keywords.InList(s)) {
                        ...
                        sc.ChangeState(SCE_JAVA_WORD|activitySet);
					} else if (keywords2.InList(s)) {
                        sc.ChangeState(SCE_JAVA_WORD2|activitySet);
					} else if (keywords4.InList(s)) {
                        sc.ChangeState(SCE_JAVA_GLOBALCLASS|activitySet);
					} else {
					...
					if (literalString || sc.ch == '\'') {
						...
						if (valid) {
							if (literalString) {
								if (raw) {
                                    // Set the style of the string prefix to SCE_JAVA_STRINGRAW but then change to
                                    // SCE_JAVA_DEFAULT as that allows the raw string start code to run.
                                    sc.ChangeState(SCE_JAVA_STRINGRAW|activitySet);
                                    sc.SetState(SCE_JAVA_DEFAULT|activitySet);
								} else {
                                    sc.ChangeState(SCE_JAVA_STRING|activitySet);
								}
							} else {
                                sc.ChangeState(SCE_JAVA_CHARACTER|activitySet);
							}
						} else {
                            sc.SetState(SCE_JAVA_DEFAULT | activitySet);
						}
            ...
            case SCE_JAVA_PREPROCESSOR:
				if (options.stylingWithinPreprocessor) {
					...
					if ((isIncludePreprocessor && sc.Match('<')) || sc.Match('\"')) {
						...
						if (sc.Match("/**") || sc.Match("/*!")) {
                            sc.SetState(SCE_JAVA_PREPROCESSORCOMMENTDOC|activitySet);
						} else {
                            sc.SetState(SCE_JAVA_PREPROCESSORCOMMENT|activitySet);
						}
						sc.Forward();	// Eat the *
					} else if (sc.Match('/', '/')) {
                        sc.SetState(SCE_JAVA_DEFAULT|activitySet);
				...
				break;
            case SCE_JAVA_PREPROCESSORCOMMENT:
            case SCE_JAVA_PREPROCESSORCOMMENTDOC:
				if (sc.Match('*', '/')) {
					sc.Forward();
                    sc.ForwardSetState(SCE_JAVA_PREPROCESSOR|activitySet);
					continue;	// Without advancing in case of '\'.
				}
				break;
            case SCE_JAVA_COMMENT:
				if (sc.Match('*', '/')) {
					sc.Forward();
                    sc.ForwardSetState(SCE_JAVA_DEFAULT|activitySet);
				} else {
                    styleBeforeTaskMarker = SCE_JAVA_COMMENT;
					highlightTaskMarker(sc, styler, activitySet, markerList, caseSensitive);
				}
				break;
            case SCE_JAVA_COMMENTDOC:
				if (sc.Match('*', '/')) {
					sc.Forward();
                    sc.ForwardSetState(SCE_JAVA_DEFAULT|activitySet);
					...
					if ((IsASpace(sc.chPrev) || sc.chPrev == '*') && (!IsASpace(sc.chNext))) {
                        styleBeforeDCKeyword = SCE_JAVA_COMMENTDOC;
                        sc.SetState(SCE_JAVA_COMMENTDOCKEYWORD|activitySet);
					}
				} else if ((sc.ch == '<' && sc.chNext != '/')
							|| (sc.ch == '/' && sc.chPrev == '<')) { // XML comment style
                    styleBeforeDCKeyword = SCE_JAVA_COMMENTDOC;
                    sc.ForwardSetState(SCE_JAVA_COMMENTDOCKEYWORD | activitySet);
				}
				break;
            case SCE_JAVA_COMMENTLINE:
				if (sc.atLineStart && !continuationLine) {
                    ...
                    styleBeforeTaskMarker = SCE_JAVA_COMMENTLINE;
					highlightTaskMarker(sc, styler, activitySet, markerList, caseSensitive);
				}
				break;
            case SCE_JAVA_COMMENTLINEDOC:
				if (sc.atLineStart && !continuationLine) {
					...
					if ((IsASpace(sc.chPrev) || sc.chPrev == '/' || sc.chPrev == '!') && (!IsASpace(sc.chNext))) {
                        styleBeforeDCKeyword = SCE_JAVA_COMMENTLINEDOC;
                        sc.SetState(SCE_JAVA_COMMENTDOCKEYWORD|activitySet);
					}
				} else if ((sc.ch == '<' && sc.chNext != '/')
							|| (sc.ch == '/' && sc.chPrev == '<')) { // XML comment style
				...
				break;
            case SCE_JAVA_COMMENTDOCKEYWORD:
                if ((styleBeforeDCKeyword == SCE_JAVA_COMMENTDOC) && sc.Match('*', '/')) {
                    sc.ChangeState(SCE_JAVA_COMMENTDOCKEYWORDERROR);
					sc.Forward();
                    sc.ForwardSetState(SCE_JAVA_DEFAULT|activitySet);
					...
					if (!(IsASpace(sc.ch) || (sc.ch == 0))) {
                        sc.ChangeState(SCE_JAVA_COMMENTDOCKEYWORDERROR|activitySet);
            ...
            case SCE_JAVA_STRING:
				if (sc.atLineEnd) {
                    sc.ChangeState(SCE_JAVA_STRINGEOL|activitySet);
					...
					if (options.escapeSequence) {
                        sc.SetState(SCE_JAVA_ESCAPESEQUENCE|activitySet);
				...
				break;
            case SCE_JAVA_ESCAPESEQUENCE:
				escapeSeq.consumeDigit();
				if (!escapeSeq.atEscapeEnd(sc.ch)) {
					break;
				}
				if (sc.ch == '"') {
                    sc.SetState(SCE_JAVA_STRING|activitySet);
                    sc.ForwardSetState(SCE_JAVA_DEFAULT|activitySet);
				} else if (sc.ch == '\\') {
					escapeSeq.resetEscapeState(sc.chNext);
					sc.Forward();
				...
				break;
            case SCE_JAVA_HASHQUOTEDSTRING:
				if (sc.ch == '\\') {
            ...
            case SCE_JAVA_REGEX:
				...
				break;
            case SCE_JAVA_STRINGEOL:
				if (sc.atLineStart) {
                    sc.SetState(SCE_JAVA_DEFAULT|activitySet);
				}
				break;
            case SCE_JAVA_VERBATIM:
        ...
        if (MaskActive(sc.state) == SCE_JAVA_DEFAULT) {
			if (sc.Match('@', '\"')) {
                sc.SetState(SCE_JAVA_VERBATIM|activitySet);
				sc.Forward();
			} else if (options.triplequotedStrings && sc.Match(R"(""")")) {
                sc.SetState(SCE_JAVA_TRIPLEVERBATIM|activitySet);
				sc.Forward(2);
			} else if (options.hashquotedStrings && sc.Match('#', '\"')) {
                sc.SetState(SCE_JAVA_HASHQUOTEDSTRING|activitySet);
				sc.Forward();
			} else if (options.backQuotedStrings && sc.Match('`')) {
                sc.SetState(SCE_JAVA_STRINGRAW|activitySet);
				rawStringTerminator = "`";
			} else if (IsADigit(sc.ch) || (sc.ch == '.' && IsADigit(sc.chNext))) {
				if (lastWordWasUUID) {
                    sc.SetState(SCE_JAVA_UUID|activitySet);
					lastWordWasUUID = false;
				} else {
                    sc.SetState(SCE_JAVA_NUMBER|activitySet);
				}
				...
				if (lastWordWasUUID) {
                    ...
                    sc.SetState(SCE_JAVA_IDENTIFIER|activitySet);
				}
			} else if (sc.Match('/', '*')) {
				if (sc.Match("/**") || sc.Match("/*!")) {	// Support of Qt/Doxygen doc. style
                    sc.SetState(SCE_JAVA_COMMENTDOC|activitySet);
				} else {
                    sc.SetState(SCE_JAVA_COMMENT|activitySet);
				}
				sc.Forward();	// Eat the * so it isn't used for the end of the comment
			} else if (sc.Match('/', '/')) {
				if ((sc.Match("///") && !sc.Match("////")) || sc.Match("//!"))
					// Support of Qt/Doxygen doc. style
                    sc.SetState(SCE_JAVA_COMMENTLINEDOC|activitySet);
				else
                    sc.SetState(SCE_JAVA_COMMENTLINE|activitySet);
			} else if (sc.ch == '/'
				   && (setOKBeforeRE.Contains(chPrevNonWhite)
				       || followsReturnKeyword(sc, styler))
				   && (!setCouldBePostOp.Contains(chPrevNonWhite)
				       || !FollowsPostfixOperator(sc, styler))) {
                sc.SetState(SCE_JAVA_REGEX|activitySet);	// JavaScript's RegEx
				inRERange = false;
			} else if (sc.ch == '\"') {
				if (sc.chPrev == 'R') {
					styler.Flush();
                    if (MaskActive(styler.StyleAt(sc.currentPos - 1)) == SCE_JAVA_STRINGRAW) {
                        sc.SetState(SCE_JAVA_STRINGRAW|activitySet);
						rawStringTerminator = ")";
                ...
                sc.SetState(SCE_JAVA_CHARACTER|activitySet);
			} else if (sc.ch == '#' && visibleChars == 0) {
				// Preprocessor commands are alone on their line
                sc.SetState(SCE_JAVA_PREPROCESSOR|activitySet);
						...
						if (sc.Match("ifdef") || sc.Match("ifndef")) {
							...
							if (preproc.ValidLevel()) {
								...
								if (!preproc.CurrentIfTaken()) {
                                    ...
                                    assert(sc.state == (SCE_JAVA_PREPROCESSOR | inactiveFlag));
									preproc.InvertCurrentLevel();
									activitySet = preproc.ActiveState();
									// If following is active then show "else" as active
									if (!activitySet)
                                        sc.ChangeState(SCE_JAVA_PREPROCESSOR);
								} else if (preproc.IsActive()) {
									// Active -> inactive
                                    assert(sc.state == SCE_JAVA_PREPROCESSOR);
									preproc.InvertCurrentLevel();
									activitySet = preproc.ActiveState();
							...
							preproc.EndSection();
							activitySet = preproc.ActiveState();
                            sc.ChangeState(SCE_JAVA_PREPROCESSOR|activitySet);
                ...
                sc.SetState(SCE_JAVA_OPERATOR|activitySet);
			}
		}
...
void SCI_METHOD LexerJAVA::Fold(Sci_PositionU startPos, Sci_Position length, int initStyle, IDocument *pAccess) {
	...
	for (Sci_PositionU i = startPos; i < endPos; i++) {
        ...
        if ((style == SCE_JAVA_COMMENTLINE) || (style == SCE_JAVA_COMMENTLINEDOC))
			inLineComment = true;
        ...
        if (options.foldComment && options.foldCommentExplicit && ((style == SCE_JAVA_COMMENTLINE) || options.foldExplicitAnywhere)) {
        ...
        if (options.foldPreprocessor && (style == SCE_JAVA_PREPROCESSOR)) {
			if (ch == '#') {
				Sci_PositionU j = i + 1;
        ...
        if (options.foldSyntaxBased && (style == SCE_JAVA_OPERATOR)) {
			if (ch == '{' || ch == '[' || ch == '(') {
...
void LexerJAVA::EvaluateTokens(std::vector<std::string> &tokens, const SymbolTable &preprocessorDefinitions) {
...
std::vector<std::string> LexerJAVA::Tokenize(const std::string &expr) const {
	// Break into tokens
...
bool LexerJAVA::EvaluateExpression(const std::string &expr, const SymbolTable &preprocessorDefinitions) {
...
LexerModule lmJAVA(SCLEX_JAVA, LexerJAVA::LexerFactoryJAVA, "java", javaWordLists);
LexerModule lmJAVANoCase(SCLEX_JAVANOCASE, LexerJAVA::LexerFactoryJAVAInsensitive, "javanocase", javaWordLists);
