...
void ResourceSettingWidget::updateSettings()
{
    ...
    targetLocationEdit->setText(settings->value(kActiveGroup, kActiveTarget).toString());
...
QStringList ResourceSettingWidget::selectedFileList(NodeItem *item)
{
    QStringList selList;
...
void ResourceSettingWidget::handleProjectChanged()
{
    ...
    if (targetLocationEdit->text().isEmpty()) {
...
bool ResourceSettingWidget::apply()
{
    ...
    if (target.isEmpty() || !Utils::isValidPath(target)) {
        targetLocationEdit->setAlert(true);
        targetLocationEdit->showAlertMessage(tr("Please input a valid path"));
        targetLocationEdit->setFocus();
        targetLocationEdit->lineEdit()->selectAll();
        return false;
    }
    setting->setValue(kActiveGroup, kActiveTarget, target);

    return true;
}

QStringList ResourceSettingWidget::selectedFileList()
{
...
QString ResourceSettingWidget::selectedProject()
{
    const auto &prjInfo = projectCB->currentData().value<dpfservice::ProjectInfo>();
