...
void SmartUTWidget::initConnection()
{
    ...
    connect(SmartUTManager::instance(), &SmartUTManager::itemStateChanged, prjView, &ProjectTreeView::updateItem);
}
...
void SmartUTWidget::fillProjectView(const QString &workspace, const QStringList &fileList)
{
    ...
    const auto &target = setting->value(kActiveGroup, kActiveTarget).toString();
