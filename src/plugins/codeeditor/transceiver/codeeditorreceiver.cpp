...
void CodeEditorReceiver::eventProcess(const dpf::Event &event)
{
    ...
    } else if (event.data() == editor.removeDebugLine.name) {
        return EditorCallProxy::instance()->reqRemoveDebugLine();
    } else if (event.data() == editor.setDebugLine.name) {
        QString filePath = event.property(editor.setDebugLine.pKeys[0]).toString();
        int line = event.property(editor.setDebugLine.pKeys[1]).toInt();
    ...
    } else if (event.data() == editor.addBreakpoint.name) {
    ...
    } else if (event.data() == editor.removeBreakpoint.name) {
