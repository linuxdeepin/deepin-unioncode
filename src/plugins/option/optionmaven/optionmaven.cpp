 ...
 */
#include "optionmaven.h"
#include "mainframe/optionmavengenerator.h"
...
void OptionMaven::initialize()
{
    DPF_USE_NAMESPACE
}

bool OptionMaven::start()
{
    auto &ctx = dpfInstance.serviceContext();
    OptionService *optionService = ctx.service<OptionService>(OptionService::name());
    if (!optionService) {
        qCritical() << "Failed, not found OptionMaven service!";
        abort();
    }
...
dpf::Plugin::ShutdownFlag OptionMaven::stop()
{
    return Sync;
}
