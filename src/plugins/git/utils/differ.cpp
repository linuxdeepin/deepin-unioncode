...
static int commonOverlap(const QString &text1, const QString &text2)
{
    ...
    const int maxCount = qMin(text1Count, text2Count);
    while (i < maxCount) {
        if (text1.mid(text1Count - maxCount + i) == text2.left(maxCount - i))
            return maxCount - i;
        i++;
    }
    return 0;
...
{
    currentDiffMode = settedDiffMode;
...
QList<Diff> Differ::unifiedDiff(const QString &text1, const QString &text2)
{
    ...
    const DiffMode diffMode = currentDiffMode;
    currentDiffMode = CharMode;
...
QList<Diff> Differ::preprocess2AndDiff(const QString &text1, const QString &text2)
{
    ...
    if (currentDiffMode != Differ::CharMode && text1.size() > 80 && text2.size() > 80)
...
QList<Diff> Differ::diffMyers(const QString &text1, const QString &text2)
{
    ...
    int kMaxReverse = D;
    for (int d = 0; d <= D; d++) {
        if (diffFuture && diffFuture->isCanceled()) {
            delete [] forwardV;
            delete [] reverseV;
            return {};
        }
...
QList<Diff> Differ::diffNonCharMode(const QString &text1, const QString &text2)
{
    ...
    DiffMode diffMode = currentDiffMode;
    currentDiffMode = CharMode;
...
int Differ::findSubtextEnd(const QString &text,
                                  int subtextStart)
{
    if (currentDiffMode == Differ::LineMode) {
        int subtextEnd = text.indexOf('\n', subtextStart);
        if (subtextEnd == -1)
            subtextEnd = text.size() - 1;
        return ++subtextEnd;
    } else if (currentDiffMode == Differ::WordMode) {
